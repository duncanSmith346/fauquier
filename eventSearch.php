<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (isset($args['submitName'])) {
            if (!wereRequiredFieldsSubmitted($args, array('eventname'))) {
                echo 'missing form data';
                die();
            }
            $events = find_event($args['eventname']);
            $search = 'Results for Search by Name: "' . htmlspecialchars($_POST['eventname']) . '"';
...
<html>
    <head>
        ...
        <title>FUMC VMS | Event Search</title>
    </head>
    <body>
                        ...
                        foreach ($events as $event) {
                            ...
                            echo "
                                <table class='event'>
                                    <thead>
                                        <tr>
                                            <th colspan='2' data-event-id='" . $event['id'] . "'>" . $event['eventname'] . "</th>
                                        </tr>
                                    </thead>
                                    <tbody>
