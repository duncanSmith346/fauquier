...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_course'])) {
    if (!empty($_POST['selected_course_id'])) {
        ...
        // Ensure retrieve_course didn't fail
        if (!$courseDetails) {
            header("Location: updateCourse.php?error=course_not_found");
            exit();
        }

        $trainingPeriod = isset($courseDetails['periodId']) ? $courseDetails['periodId'] : null;
        
        if ($trainingPeriod != null && $trainingPeriod != 0) {
...
<body>
    ...
    <fieldset>
        ...
        <label for="selected_course_id">Select a Course:</label>
        <select name="selected_course_id" id="selected_course_id" required>
            <option value="">--Select a Course--</option>
            <?php foreach ($courses as $course) : ?>
                <option value="<?php echo htmlspecialchars($course['id']); ?>">
                    <?php echo htmlspecialchars($course['name']); ?> - <?php echo htmlspecialchars($course['abbrevName']); ?>
                </option>
            <?php endforeach; ?>
        </select>
