...
class Person {
	...
	function __construct($f, $l, $v, $a, $c, $s, $z, $pp, $p1, $p1t, $p2, $p2t, $e, $ts, 
                     $comp, $cam, $tran, $cn, $cpn, $rel, $ct, $t, $st, $cntm, $pos, 
                     $credithours, $comm, $mot, $spe, $convictions, $av, $sch, $hrs, 
                     $bd, $sd, $hdyh, $notes, $pass, $suns, $sune, $mons, $mone, $tues, 
                     $tuee, $weds, $wede, $thus, $thue, $fris, $frie, $sats, $sate, 
                     $mcp, $gender)  {
		$this->id = $e;
		$this->start_date = $sd;
		$this->venue = $v;
    ...
    if (trim($t) !== "") {
        ...
        foreach ($roleArray as $role) {
            ...
            $this->type[] = $role;
            if (isset($accessLevelsByRole[$role])) {
                $maxAccess = max($maxAccess, $accessLevelsByRole[$role]);
            }
        }
        error_log("DEBUG \$t is: [$t]");

        $this->access_level = $maxAccess;
        error_log("DEBUG: \$t = [$t], roles = " . print_r($this->type, true) . 
                  ", assigned access level = $maxAccess");
    } else {
        $this->type = [];
        error_log("DEBUG \$t is: [$t]");
        $this->access_level = 0;
